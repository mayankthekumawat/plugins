...
Transcripts live in the active runtime's workspace-scoped conversation history when it exposes one. Use only the active workspace's history. Every record is one chat message.

If the runtime exposes no workspace-scoped history, use the current conversation, live repository state, and shared records that are actually available. State the history gap in the brief.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time rather than UUID name, search the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. If the runtime has no subagent capability, inspect the slices sequentially in the parent. The raw transcripts stay out of the main context; keep only their findings.
