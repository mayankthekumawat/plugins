...
Create `orchestrate/<project-slug>/` in a persistent workspace store exposed by the runtime, or under `.pstack/orchestrate/<project-slug>/` when none exists. Record the resolved path in the run. Every file has exactly one writer; owners publish facts, readers aggregate at read time. Resolve `scripts/orch/orch.ts` relative to the installed `poteto-mode` skill and use it with Bun for bookkeeping, written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI. If Bun is unavailable, maintain those plain files directly with the same shapes.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe the runtime's user-input mechanism state.
...
A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the remote-isolation default and the local exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it through the runtime's recurring or wake mechanism when available, with a long heartbeat fallback), and before a human report. Without that capability, poll synchronously while the session remains active and leave the exact resume predicate if a later wake is required. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run remotely when supported; a local restack at this scale takes the laptop down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, and the runtime's agent status surface. Transcript mtime is not liveness.
...
- After a runtime restart: local agents may be dead while remote work continues. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach remote work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
