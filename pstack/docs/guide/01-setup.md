...
In this page you install the skills, pick which models pstack uses, and run your first task. Setup is one copy plus a short conversation.

## Install the skills

Copy or link the directories under `pstack/skills/` into a standards-compatible Agent Skills directory:

```text
.agents/skills/
```

Use `~/.agents/skills/` for a user-level installation. If your runtime uses another skill directory, place them there instead.
...
[`setup-pstack`](../../skills/setup-pstack/SKILL.md) detects the models you have access to, shows you each role (code delegates, judgment, the review panels), and asks what you want. Answer the questions. It writes `.pstack/models.yaml`, a small rule every pstack skill reads.
